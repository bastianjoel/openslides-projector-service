...
func newProjector(id int, db *datastore.Datastore) (*projector, error) {
	projectorQuery := datastore.Collection(db, &models.Projector{}).SetIds(id).AsSingle()
	...
	if err = p.updateFullContent(projectorData); err != nil {
...
func (p *projector) subscribeProjector() {
	projectorSub := datastore.Collection(p.db, &models.Projector{}).SetIds(p.id).AsSingle().Subscribe()
...
func (p *projector) getProjectionSubscription(id int) (<-chan string, error) {
	data, err := datastore.Collection(p.db, &models.Projection{}).SetIds(id).AsSingle().Run()
	if err != nil {
		return nil, fmt.Errorf("error fetching projector from db %w", err)
	}

	projectionData := data.(*models.Projection)
	return p.slideRouter.SubscribeContent(nil, projectionData), nil
}
